...
mapeo_subcategorias = {
    "Preentrenamiento y Óxido Nítrico": [
        "Pre-Workout",
        "Pre-entreno",
        "Preentrenamiento",
        "Preentrenamiento y Óxido Nítrico",
    ...
    "Proteína Whey": [
        ...
        "Proteínas Aisladas de Suero",
        "Proteínas Concentradas de Suero",
        " Proteína Whey ",
    ],
    "Aislado de Whey y Whey Nativa": [
        "Aislado de Whey y Whey Nativa",
        "Protein",
        "Proteínas",
        "Batidos de proteínas en Polvo",
    ...
    "Caseína e Proteína de liberación lenta": [
        "Caseína e Proteína de liberación lenta",
        "Caseínas",
        "Proteínas Liberación Secuencial",
    ],
    ...
    "Prohormonales": [
        "Prohormonales",
        "Control/Reducir Cortisol",
        "Control/Reducir Estrógenos",
        "Pro-Hormona Crecimiento",
    ],
}
...
mapeo_categorias = {
    ...
    "Quemadores de grasa y definición": [
        "Termogénicos",
        "Quemadores de Grasa sin Estimulantes",
        "Cremas Quemagrasas",
        "CLA",
...
def get_imagen_raw(url, driver):
    driver.get(url)
    url_imagen = driver.find_element(
        By.XPATH, "//a[@class='show-gallery']"
    ).get_attribute("href")
...
def get_reviews_raw(url, driver):
    driver.get(url)
    reviews = []
    reviews_url = driver.find_element(
        By.XPATH, "//iframe[@id='looxReviewsFrame']"
    ).get_attribute("src")
...
def get_reviews_big(url, driver):
    ...
    try:
        ...
        try:
            reviews_tab = driver.find_element(
                By.XPATH, "//li[contains(@class, 'tab') and contains(.,'Reviews')]"
            )
        except NoSuchElementException:
    ...
    time.sleep(3)
    try:
        reviews_element = driver.find_elements(
            By.XPATH, "//div[contains(@class, 'content-review')]"
        )
...
def get_ingredientes_big(url, driver):
    ...
    time.sleep(3)
    try:
        info_nutricional = driver.find_element(
            By.XPATH, "//a[contains(@href, '#tab-2')]"
        )
    except NoSuchElementException:
        try:
            info_nutricional = driver.find_element(
                By.XPATH,
                "//li[contains(@class, 'tab') and contains(.,'Info. Nutricional')]",
            )
        except NoSuchElementException:
    ...
    time.sleep(3)
    try:
        ingredientes_element = driver.find_element(
            By.CSS_SELECTOR, "table.nutri_info"
        ).find_element(
            By.XPATH, "//p[starts-with(normalize-space(), 'Ingredientes')][1]"
        )

        # Omitir elementos en etiquetas <b>
        ingredientes_texto = ingredientes_element.get_attribute("innerHTML")
        ingredientes_sin_etiquetas_b = re.sub(
            r"<(strong|b)>(.*?)</(strong|b)>", "", ingredientes_texto
        )
