...
urls_big = [
    "https://bigsupps.site/collections/aminoacids",
    "https://bigsupps.site/collections/testosterone",
    "https://bigsupps.site/collections/protein",
    "https://bigsupps.site/collections/pre-workout",
    "https://bigsupps.site/collections/fat-burner",
    "https://bigsupps.site/collections/mass-gainer",
    "https://bigsupps.site/collections/micronutrients",
    "https://bigsupps.site/collections/snacks",
]
...
def big_scrap_complete(driver, writer):
    for url in urls_big:
        ...
        try:
            num_pags = (
                s.find("div", class_="pagination has-margin-top-6 has-margin-bottom-6")
                .find_all("span")[-2]
                .text
            )
        except:
        ...
        subcategoria_scrapeada = s.find(
            "h2",
            class_="has-text-weight-bold is-uppercase is-size-2-widescreen is-size-3-tablet is-size-5-mobile has-padding-bottom-3",
        ).text.strip()
        ...
        if int(num_pags) > 1:
            for pag in range(1, int(num_pags) + 1):
                ...
                s = BeautifulSoup(f, "lxml")
                time.sleep(2)
                pList = s.find("div", class_="product-list").find_all(
                    "div", class_="product-block detail-mode-permanent fixed-width"
                )
                for p in pList:
                    try:
                        ...
                        precio = (
                            s.find("div", class_="price-text after")
                            .find("span")
                            .text.replace("€", "")
                            .replace(",", ".")
                            .strip()
                        ...
                        try:
                            stock_label = (
                                s.find("div", class_="column detail")
                                .find("span", class_="productlabel soldout")
                                .text
                            )
                            stock = False
                        except:
