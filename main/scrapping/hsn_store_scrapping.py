...
urls_hsn = [
    "/aminoacidos/aislados",
    "/aminoacidos/bcaa-s-ramificados",
    "/aminoacidos/esenciales-eaas",
    "/aminoacidos/glutamina",
    "/aminoacidos/hmb",
    "/anabolicos-naturales/suplementos-control-cortisol",
    "/anabolicos-naturales/suplementos-para-control-y-reduccion-estrogenos",
    "/anabolicos-naturales/pro-hormona-del-crecimiento",
    "/anabolicos-naturales/pro-testosterona",
    "/anabolicos-naturales/zma",
    "/barritas/proteinas",
    "/barritas/energeticas",
    "/barritas/sustitutas-de-comida",
    "/carbohidratos",
    "/controlar-peso/bloqueadores-grasas-carbohidratos",
    "/controlar-peso/inhibidores-apetito",
    "/controlar-peso/cremas-reductoras",
    "/controlar-peso/diureticos",
    "/controlar-peso/termogenicos",
    "/controlar-peso/quemadores-termogenicos-sin-estimulantes",
    "/creatina",
    "/ganadores-de-peso",
    "/intra-entrenamiento",
    "/minerales",
    "/multivitaminicos",
    "/post-entrenamiento-y-recuperacion",
    "/pre-entrenamiento",
    "/proteinas/caseina",
    "/proteinas/albumina-de-huevo",
    "/proteinas/carne",
    "/proteinas/liberacion-secuencial",
    "/proteinas/vegetales",
    "/proteinas/whey/aislados-de-suero",
    "/proteinas/whey/concentrados-de-suero",
    "/proteinas/whey/hidrolizadas",
    "/vitaminas",
]
...
def hsn_scrap_complete(driver, writer):
    for u in urls_hsn:
        ...
        try:
            ...
            num_pags = 1
        subcategoria_scrapeada = (
            s.find("div", class_="page-title category-title").find("span").text.strip()
        )
        ...
        if int(num_pags) > 1:
            for pag in range(1, num_pags + 1):
                ...
                time.sleep(3)
                try:
                    pList = s.find("ul", class_="products-grid").find_all(
                        "li", class_="item last"
                    )
                except:
                    pList = []
                    ...
                    try:
                        ...
                        try:
                            ...
                            for r in rating_divs:
                                if (
                                    r["class"][0] == "fas"
                                    and r["class"][1] == "fa-star"
                                ):
                                    rating += 1.0
                                elif (
                                    r["class"][0] == "fa"
                                    and r["class"][1] == "fa-star-half-alt"
                                ):
                                    rating += 0.5
                                else:
                                    rating += 0.0
                        ...
                        descripcion_final = (
                            s.find(
                                "div", class_="col-xs-12 no-padding-lat product_desc"
                            )
                            .find("p")
                            .text
                        )
                        ...
                        try:
                            reviews_divs = s.find(
                                "ul", class_="col-xs-12 reviews-comment-box"
                            ).find_all("li")
                            for r in reviews_divs:
                                reviews_list.append(
                                    r.find("p", class_="content-review").text
                                )
                        except:
                        ...
                        try:
                            ingredientes_aux = (
                                s.find("div", class_="table_ingredientes")
                                .find_all("p")[1]
                                .text.split(".")[0]
                            )
                            ingredientes = parse_ingredientes(ingredientes_aux)
                        except:
                            ingredientes_aux = s.find(
                                "div", class_="table_ingredientes"
                            )
                            try:
                                ingredientes = parse_ingredientes(ingredientes_aux)
                            except:
                        ...
                        for i in ingredientes:
                            i = i.strip()
                            ingrediente_obj = Ingrediente.objects.get_or_create(
                                ingrediente=i
                            )[0]
                        ...
                        try:
                            existe_registro = Producto.objects.filter(
                                url=url_producto
                            ).exists()
                        ...
                        print(
                            f"Se ha producido un error al scrapear el producto: {nombre}"
                        )
                        print(f"Error: {e}")
                        continue
        else:
            ...
            for a in pList:
                try:
                    ...
                    descripcion_final = (
                        s.find("div", class_="col-xs-12 no-padding-lat product_desc")
                        .find("p")
                        .text
                    )
