...
def prozis_scrap_complete(driver, writer):
    ...
    for pag in range(1, int(num_pags) + 1):
        ...
        time.sleep(4)
        productos = driver.find_element(
            By.CLASS_NAME, "row.list-container"
        ).find_elements(By.CLASS_NAME, "col.list-item")
    ...
    for url_producto in urls_productos:
        try:
            ...
            subcategoria_scrapeada = (
                driver.find_element(By.ID, "breadcrumbs")
                .find_elements(By.TAG_NAME, "a")[-2]
                .text.strip()
            )
            ...
            precio = (
                driver.find_element(By.XPATH, "//p[@class='final-price']")
                .get_attribute("data-qa")
                .replace("€", "")
                .replace(",", ".")
                .strip()
            )
            ...
            rating = (
                driver.find_element(By.CLASS_NAME, "prz-blk-content-rating")
            ...
            url_imagen = (
                driver.find_element(By.CLASS_NAME, "first-column")
            ...
            for element in elements:
                ...
                if not element.find_elements(By.CSS_SELECTOR, "i.prz-fullscreen"):
                    ...
                    if element.find_elements(
                        By.XPATH, ".//span[contains(text(), 'Información Nutricional')]"
                    ) or element.find_elements(
                        By.XPATH,
            ...
            try:
                ingredientes_text = (
                    driver.find_element(By.CLASS_NAME, "nut-other-ingredients")
            ...
            driver.find_element(
                By.ID, "contentCloseBtn"
            ).click()  # Cierra menu info nutricional
            time.sleep(1)
            ...
            for element in elements:
                ...
                if not element.find_elements(By.CSS_SELECTOR, "i.prz-fullscreen"):
                    ...
                    if element.find_elements(
                        By.XPATH, ".//span[contains(text(), 'Opiniones generales')]"
                    ):
                        element.click()
                        break
            time.sleep(2)
            try:
                reviews_divs = driver.find_element(
                    By.CLASS_NAME, "reviews-section.customer-reviews"
                ).find_elements(By.CLASS_NAME, "review-detailed")
                reviews_list = []
                for review in reviews_divs:
                    reviews_list.append(
                        review.find_element(By.CLASS_NAME, "review-content").text
                    )
            ...
            try:
                driver.find_element(
                    By.CLASS_NAME,
                    "prz-blk-content.pdp-block-horizontal.prz-blk-content-with-img",
                ).click()  # Abre menu descripciones
                time.sleep(2)
                descripcion = ""
                table_descripciones = driver.find_element(
                    By.CLASS_NAME, "p-0"
