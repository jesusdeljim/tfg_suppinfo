...
def myprotein_scrap_categories():
    ...
    categorias = s.find("div", class_="responsiveFlyoutMenu_mobilePanelContainer").find("li", class_="responsiveFlyoutMenu_levelOneItem-slide")
    print(categorias)
...
def myprotein_scrap_proteins():
    #scrapping de la seccion de proteina
        ...
        for pag in range(1,4):
            ...
            s = BeautifulSoup(f, "lxml")
            products_list = s.find("ul", class_="productListProducts_products").find_all("li")
            for product in products_list:
                product_url = product.find("a", class_="athenaProductBlock_linkImage")['href']
